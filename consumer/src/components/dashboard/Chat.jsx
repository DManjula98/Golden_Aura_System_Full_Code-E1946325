...
const Chat = () => {
    ...
    useEffect(() => {
        socket.emit('add_user', userInformation.id, userInformation)
    }, [])
    ...
    useEffect(() => {
        ...
        if (receverMessage) {
            if (sellerId === receverMessage.senderId && userInformation.id === receverMessage.receverId) {
                ...
                <div className='w-full flex flex-col text-slate-600 py-4 h-[400px] pr-3'>
                    {
                       my_friends.map((f, i) => <Link to={`/dashboard/chat/${f.fdId}`} key={i} className={`flex gap-2 justify-start items-center pl-2 py-[5px]`} >
                            <div className='w-[30px] h-[30px] rounded-full relative'>
                                ...
                                <img src="http://localhost:3000/images/manager.jpg" alt="" />
                            </div>
                            <span>{f.name}</span>
                        </Link>)
                    }
           ...
           { currentFd ? (
                                ...
                                if (currentFd?.fdId !== m.receverId) {
                                    return(
                                        <div key={i} ref={scrollRef} className='w-full flex gap-2 justify-start items-center text-[14px]'>
                                            <img className='w-[30px] h-[30px] ' src="http://localhost:3000/images/manager.jpg" alt="" />
                                    ...
                                    return(
                                        <div key={i} ref={scrollRef} className='w-full flex gap-2 justify-end items-center text-[14px]'>
                                            <img className='w-[30px] h-[30px] ' src="http://localhost:3000/images/customer.png" alt="" />
                    ...
                    <div className='flex p-2 justify-between items-center w-full'>
                        <div className='w-[40px] h-[40px] border p-2 justify-center items-center flex rounded-full'>
                            <label  htmlFor=""><AiOutlinePlus /></label>
