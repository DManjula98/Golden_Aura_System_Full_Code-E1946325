...
const EditProduct = () => {
  ...
  const changeImage = (img, files) => {
    if (files.length > 0) {
      ...
      dispatch(
        ...
        .then(() => {
          console.log("Image updated, fetching product...");
          return dispatch(get_product(productId));
        })
        .then(({ payload }) => {
          console.log("Updated product:", payload);
          if (payload.product && payload.product.images) {
            setImageShow(payload.product.images); 
          }
        })
        .catch((error) => {
  ...
  const update = (e) => {
    ...
    const obj = {
      ...
      mdescription: state.mdescription,
      ...
      <div className="w-full p-4 bg-[#283046] rounded-md">
        ...
        <div>
          <form onSubmit={update}>
            ...
            <div className="flex flex-col w-full gap-1 text-[#d0d2d6] mb-5">
              <label htmlFor="mdescription">Benifits</label>
              <textarea
                rows={4}
                className="px-4 py-2 focus:border-indigo-500 outline-none bg-[#283046] border border-slate-700 rounded-md text-[#d0d2d6]"
                onChange={inputHandle}
