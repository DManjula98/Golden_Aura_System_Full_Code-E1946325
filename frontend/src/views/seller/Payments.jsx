...
const Payments = () => {
  ...
  const Rows = ({ index, style }) => {
    return (
      <div style={style} className="flex text-sm">
        <div className="w-[25%] p-2 whitespace-nowrap">{index + 1}</div>
        <div className="w-[25%] p-2 whitespace-nowrap">
          {formatPrice(successWithdrows[index]?.amount)}
        </div>
        <div className="w-[25%] p-2 whitespace-nowrap">
          <span className="py-[1px] px-[5px] bg-slate-700 text-blue-500 rounded-md text-xs">
  ...
  useEffect(() => {
    dispatch(get_seller_payemt_details(userInformaion._id));
  }, []);
  ...
  const sendRequest = (e) => {
    ...
    if (availableAmount - amount > 10) {
      dispatch(
        send_withdrowal_request({ amount, sellerId: userInformaion._id })
      );
      setAmount(0);
    } else {
  ...
  return (
    <div className="px-2 md:px-7 py-5">
      <div className="w-full  grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
        <div className="flex justify-between items-center ps-5 bg-[#283046] rounded-md gap-3 shadow-lg">
          <div className="flex flex-col justify-start items-start text-[#d0d2d6]">
            <h2 className="text-lg font-bold">{formatPrice(totalAmount)}</h2>
            <span className="text-sm font-normal">Total Sales</span>
          </div>
          <div className="w-[46px] h-[44px] rounded-full bg-[#28c76f1f] flex justify-center items-center text-sm ">
        ...
        <div className="flex justify-between items-center ps-5 bg-[#283046] rounded-md gap-3">
          ...
          <div className="w-[46px] h-[44px] rounded-full bg-[#e000e81f] flex justify-center items-center text-sm ">
        ...
        <div className="flex justify-between items-center ps-5 bg-[#283046] rounded-md gap-3">
          <div className="flex flex-col justify-start items-start text-[#d0d2d6]">
            <h2 className="text-lg font-bold">{formatPrice(withdrowAmount)}</h2>
            <span className="text-sm font-medium">Withdrawal Amount</span>
          </div>
          <div className="w-[46px] h-[44px] rounded-full bg-[#7367f01f] flex justify-center items-center text-sm ">
        ...
        <div className="flex justify-between items-center ps-5 bg-[#283046] rounded-md gap-3">
          <div className="flex flex-col justify-start items-start text-[#d0d2d6]">
            <h2 className="text-lg font-bold">{formatPrice(pendingAmount)}</h2>
