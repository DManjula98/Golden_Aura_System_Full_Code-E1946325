...
const SellerToCustomerChat = () => {
  ...
  useEffect(() => {
    dispatch(get_customers(userInformaion._id));
  }, []);
  ...
  const send = (e) => {
    ...
    dispatch(
      send_message({
        senderId: userInformaion._id,
        receverId: customerId,
        text,
        name: userInformaion?.shopInformation?.shopName,
      })
    );
    setText("");
  };
  ...
  useEffect(() => {
    if (receverMessage) {
      if (
        ...
        userInformaion._id === receverMessage.receverId
      ) {
